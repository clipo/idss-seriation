from seriation.utils import iso_filter_graphs, interassemblage_distance_euclidean


def continuityMaximizationSeriation(self):
    """
    Wrapper function to allow the legacy IDSS code to call this with "self"
    instead of explicit arguments.  The actual implementation takes explicit
    parameter arguments, to facilitate embedding in a variety of codes, and 
    optimization analyses with test scripts, etc.  

    The current implementation is located in :meth:_continuitySeriationImpl.  
    """
    return _continuitySeriationImpl(self,self.assemblages,self.assemblageSize,self.xAssemblage,self.yAssemblage)




def _continuitySeriationImpl(self,
    assemblages,assemblageFrequencies,assemblageSize,xAssemblage,yAssemblage):
